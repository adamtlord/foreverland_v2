...
if not DEFAULT_DB_SETTINGS["PASSWORD"]:
    ...
    MYSQL_USER_PASSWD = "mysql -u%s -p'%s'" % (
        DEFAULT_DB_SETTINGS["USER"],
...
def _launch(full=False):
    ...
    DUMP_FILENAME = "launchdump-%s.sql.gz" % datetime.now().strftime(
        "%Y-%m-%d-%H-%M-%S"
    )
    run(
        "mysqldump --host=%s -u%s %s %s | gzip > /tmp/%s"
        % (
            env.database["HOST"],
            env.database["USER"],
            env.remote_mysql_pw_arg,
            env.database["NAME"],
            DUMP_FILENAME,
        )
    )
...
def syncdb():
    ...
    if confirm(
        "This may replace your db (you will get opportunity to specify which one). You sure?"
    ):
        run(
            "mysqldump --host=%s -u%s %s %s | gzip > /tmp/%s"
            % (
        ...
        local(
            "%s %s < %s"
            % (MYSQL_USER_PASSWD, DEFAULT_DB_SETTINGS["NAME"], DUMP_FILENAME_SQL)
        )
...
def sync_uploads():
    ...
    local(
        "rsync -rva adamlord@%s:webapps/foreverland_uploadsserve/ uploads"
        % (env.host_string)
    )
