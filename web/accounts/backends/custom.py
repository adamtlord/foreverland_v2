...
class CustomRegistrationView(RegistrationView):
    ...
    def register(self, request, **cleaned_data):
        ...
        if getattr(settings, "AUTOMATIC_ACTIVATION_AFTER_REGISTRATION", True):
            ...
            CustomActivationView().activate(
                request, registration_profile.activation_key
            )
...
class CustomActivationView(ActivationView):
    def activate(self, request, activation_key):
        ...
        if activated_user:
            user_activated.send(
                sender="activate_user", user=activated_user, request=request
            )
            ...
            if getattr(settings, "SEND_EMAIL_AFTER_ACTIVATION", False):
                ...
                subject = render_to_string(
                    "registration/postactivation_email_subject.txt", ctx_dict
                )
                subject = "".join(subject.splitlines())
                send_mail_using_template(
                    subject,
                    "registration/postactivation_email.html",
