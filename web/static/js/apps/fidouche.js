...
$(function($) {
  ...
  function updateFields() {
    if ($('#auto_calc').bootstrapSwitch('state') === true) {
      ...
      if ($('#id_commission_withheld').is(':checked')) {
  ...
  function processItemized() {
    ...
    if (printCosts + shipCosts !== 0) {
      $('#id_print_ship_cost').val(printCosts + shipCosts).change();
    }
    if (adCosts !== 0) {
      $('#id_ads_cost').val(adCosts).change();
    }
    if (otherCosts !== 0) {
      $('#id_other_cost').val(otherCosts.toFixed(2)).change();
    }
    _updateFields();
  }

  function sumRealPayouts() {
    var sum = 0;
    $('#payment_formset .amount input, #sub_payment_formset .amount input').each(function() {
      sum += Number($(this).val());
    });
    return sum;
  }
  ...
  $('.set-payout').on('blur', '#id_payout', function() {
    var payout = $(this).val();
    $('#payment_formset tr').each(function() {
      if ($(this).find('.member select').val()) {
        $(this).find('.amount input').val(payout).change();
      }
    });
    _updateFields();
    // $('#id_to_account').val(sumRealPayouts());
  });
