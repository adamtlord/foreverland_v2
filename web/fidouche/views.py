...
def financial_dashboard(request, template="fidouche/dashboard.html"):
    ...
    next_show = (
        Show.objects.filter(date__gte=datetime.datetime.now())
        .order_by("date")
        .select_related("venue")
        .first()
    )
...
def gig_finances(request, gig_id=None, template="fidouche/gig_finances.html"):
    ...
    PaymentFormSet = inlineformset_factory(
        Show, Payment, form=PaymentForm, extra=len(active_members), max_num=14
    )
    SubPaymentFormSet = inlineformset_factory(
        Show, SubPayment, form=SubPaymentForm, extra=1
    )
    ProductionPaymentFormSet = inlineformset_factory(
        Show, ProductionPayment, extra=1, form=ProductionPaymentForm
    )
    FiduciaryPaymentFormSet = inlineformset_factory(
        Show, FiduciaryPayment, extra=1, form=FiduciaryPaymentForm
    )
    ...
    if request.method == "POST":
        ...
        production_payment_formset = ProductionPaymentFormSet(
            request.POST, instance=gig
        )
        ...
        if (
            ...
            and sub_payment_formset.is_valid()
            and production_payment_formset.is_valid()
            and fiduciary_payment_formset.is_valid()
        ):
            form.save()
            ...
            messages.add_message(
                request,
                messages.SUCCESS,
                '<i class="fa fa-beer"></i> <strong>NICE.</strong> Gig finances updated!',
            )
            ...
            messages.add_message(
                request,
                messages.ERROR,
                '<i class="fa fa-wrench"></i> <strong>Aw, damnit.</strong> Something\'s fucked up.',
            )
    else:
        initial_data = {}
        if not gig.agent:
        ...
        if all(
            v is None for v in [sf["member"].value() for sf in payment_formset.forms]
        ):
...
def gig_finances_view(request, gig_id=None, template="fidouche/gig_finances_view.html"):
    ...
    if (
        gig.fee
        or gig.food_buyout
        or gig.travel_buyout
        or gig.lodging_buyout
        or gig.other_buyout
    ):
        buyouts = True
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.add_message(
                request,
                messages.SUCCESS,
                '<i class="fa fa-beer"></i> <strong>KA-CHING.</strong> Expense edited.',
            )
            return redirect(request.path)
    else:
        form = ExpenseForm(instance=expense)

    d = {"form": form, "expense": expense}
...
def expense_create(request, template="fidouche/expense_create.html"):
    ...
    if request.method == "POST":
        ...
        messages.add_message(
            request,
            messages.SUCCESS,
            '<i class="fa fa-beer"></i> <strong>KA-CHING.</strong> Expense added.',
        )
...
def expense_delete(request, expense_id=None):
    ...
    if expense_id:
        ...
        messages.add_message(
            request,
            messages.SUCCESS,
            '<i class="fa fa-beer"></i> <strong>Toast.</strong> Expense deleted.',
        )
    else:
        messages.add_message(
            request,
            messages.WARNING,
            '<i class="fa fa-warning"></i> <strong>HUH?</strong> That\'s not a thing.',
        )
    if expense_date:
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.add_message(
                request,
                messages.SUCCESS,
                '<i class="fa fa-beer"></i> <strong>KA-CHING.</strong> Income edited.',
            )
            return redirect(request.path)
    else:
        form = IncomeForm(instance=income)

    d = {"form": form, "income": income}
...
def income_create(request, template="fidouche/income_create.html"):
    ...
    if request.method == "POST":
        ...
        messages.add_message(
            request,
            messages.SUCCESS,
            '<i class="fa fa-beer"></i> <strong>KA-CHING.</strong> Income added.',
        )
        if request.POST.get("date"):
            year = datetime.datetime.strptime(request.POST.get("date"), "%Y-%m-%d").year
...
def income_delete(request, income_id=None):
    ...
    if income_id:
        ...
        messages.add_message(
            request,
            messages.SUCCESS,
            '<i class="fa fa-beer"></i> <strong>Toast.</strong> Income deleted.',
        )
    else:
        messages.add_message(
            request,
...
def tax_reports(request, template="fidouche/tax_reports.html"):
    ...
    if start and end:
        ...
        partners = partners.exclude(date_partner_joined__gt=end_date).exclude(
            date_partner_left__lt=start_date
        )
        ...
        for person in non_partner_payments:
            ...
            non_partner_payments[person]["total"] = sum(
                non_partner_payments[person]["total"]
            )
        ...
        for category in expense_payments:
            expense_payments[category]["total"] = sum(
                expense_payments[category]["total"]
            )
        ...
        d.update(
            {
                ...
                "expense_and_production_total": totalExpensePayments
                + total_production_payments
                + total_fiduciary_payments,
            }
        )
        ...
        for expense in allPayments:
            ...
            if hasattr(expense, "new_category"):
                if (
                    expense.new_category.tax_category.name
                    in vendors[expense.payee]["categories"]
                ):
                    vendors[expense.payee]["categories"][
                        expense.new_category.tax_category.name
                ...
                if "Fiduciary" in vendors[expense.payee]["categories"]:
                    vendors[expense.payee]["categories"]["Fiduciary"].append(
                        expense.amount
                    )
                else:
        ...
        for vendor in vendors:
            ...
            for category in vendors[vendor]["categories"]:
                vendors[vendor]["categories"][category] = sum(
                    vendors[vendor]["categories"][category]
                )

        d.update({"vendor_payments": vendors, "all_vendors_total": sum(vendors_total)})
        ...
        d["expense_grand_total"] = (
            d["expense_and_production_total"]
            + d["total_commission_payments"]
            + d["non_partners_total"]
        )
        d["profit_before_partners"] = d["total_income"] - d["expense_grand_total"]
...
@login_required
def production_payments(
    request, company_id=None, template="fidouche/vendor_payments.html"
):
    """View finance reports for a given timeframe/member"""
    ...
    if request.method == "POST":
        ...
        if expense_formset.is_valid():
            ...
            messages.add_message(
                request,
                messages.SUCCESS,
                '<i class="fa fa-beer"></i> <strong>NICE.</strong> Tour finances updated!',
            )
            return redirect(request.path)
        else:
            messages.add_message(
